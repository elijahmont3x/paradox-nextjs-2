...
export interface Tier {
    ...
    distribution: {             // How taxes are distributed
...
function validateDistribution(detail: DistributionDetail, totalTax: number, type: 'Buy' | 'Sell', tierId: number) {
    ...
    if (Math.abs(sum - totalTax) > 0.01) {
        console.warn(`Tier ${tierId} ${type} distribution (${sum}%) does not match total tax (${totalTax}%)`);
    }
}

tierData.forEach(tier => {
    validateDistribution(tier.distribution.buy, tier.taxes.buy, 'Buy', tier.id);
    validateDistribution(tier.distribution.sell, tier.taxes.sell, 'Sell', tier.id);
});
